...
Use the single upload entry: one file opens the single-file editor and two or more automatically open the shared queue. Drag/drop follows the same rule. There is no upfront batch-mode button. Choose other files clears the queue after discard confirmation and returns to the upload entry. Once in a batch workspace, adding files appends tasks without switching modes or losing results. A merge group contains 2–10 clips for one output; another group adds a second merge task. One local SVG file is loaded into the code editor; multiple SVG files use the queue. Existing SVG code and URL controls remain available. The optional URL downloader is not a file-upload tool and its one-use account policy is unchanged. See BATCH_WORKSPACE_UX.md for selection, previews and ZIP export.
